...
function UpgradeItem({grade}){
    ...
    return<div>
        <div onMouseMove={handleMouseMove} onMouseEnter={(()=>{setBlurb(hidden>2?true:false)})} onMouseLeave={(()=>{setBlurb(false)})}>
            <img className="gridImage" name="gridImage" src={picture} onClick={()=>(console.log(grade["index"]))}/>
        </div>
        <div id = {"cursor"} name="noBlurb" className={blurb ? "blurb" : "noBlurb"}>

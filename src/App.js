...
upgrades.forEach((thing)=>{
  ...
  function restart(){
    localStorage.remove('mainStoreItems', { path: '/' })
    localStorage.remove('upgrades', { path: '/' })
    localStorage.remove('upgrades_two', { path: '/' })
    localStorage.remove('upgrades_three', { path: '/' })
    localStorage.remove('everything', { path: '/' })
    setYouSure(false)
        ...
        <StoreItemsContext.Provider value={mainStoreItems}>
          ...
          <h2 className="userTitle">Sarah Smiler</h2>
            ...
            <img src={statistics} alt="stats" className="statistics" onClick={openStatisticsFunc}/>
              {openSettings ? <Settings handleShowAutoSave={handleShowAutoSave} autoSaveOnScreen={autoSaveOnScreen} handleAutoSaveAtAll={handleAutoSaveAtAll} autoSaveAtAll={autoSaveAtAll}/>:openStatistics ? <Statistics upgrades={upgrades} clicksSoFar={clicksSoFar} soFar={soFar} clicks={clicks}/>:<AutomatedClicks />}
